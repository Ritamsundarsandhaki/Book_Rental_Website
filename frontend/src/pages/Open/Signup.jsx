    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {Object.keys(formData).map((field, index) => (
            <motion.div key={field} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}>
              <input
                ...
                className={`w-full p-3 border rounded-lg focus:ring-2 transition-all bg-gray-50 text-gray-700 ${errors[field] ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-green-500"}`}
              />
          ...
          <motion.button
            type="submit"
            className={`w-full bg-green-500 text-white py-3 rounded-lg hover:bg-green-600 transition-all duration-300 shadow-md ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
            disabled={loading}
